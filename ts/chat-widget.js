(()=>{(function(){let x=document.createElement("div"),o=x.attachShadow({mode:"open"}),v=document.createElement("style");v.textContent=`
    ...
    `,o.appendChild(f);let g=document.createElement("div");g.id="chat-widget-container",o.appendChild(g),document.body.append(x),g.innerHTML=`
      ...
      <div class="mask"></div>
    `;let p=o.getElementById("chat-input"),r=o.getElementById("chat-submit"),l=o.getElementById("chat-messages"),E=o.getElementById("chat-bubble"),I=o.getElementById("chat-popup"),L=o.getElementById("close-popup"),M=o.getElementById("expand-popop"),C="\u4F60\u597D\uFF0C\u8FD9\u91CC\u662F\u535A\u5BA2 [\u201C\u5143\u89C6\u89D2\u201D](https://blog.yuanpei.me) \u7684 AI \u52A9\u624B\uFF0C\u4F60\u53EF\u4EE5\u4ECE\u8FD9\u91CC\u4E86\u89E3\u535A\u4E3B\u7684\u4FE1\u606F\uFF0C\u5BF9\u535A\u5BA2\u7684\u5185\u5BB9\u8FDB\u884C\u68C0\u7D22\u548C\u63D0\u95EE\u3002";r.addEventListener("click",async function(){let t=p.value.trim();t&&(l.scrollTop=l.scrollHeight,p.value="",await y(t))}),p.addEventListener("keyup",function(t){t.key==="Enter"&&r.click()}),E.addEventListener("click",function(){b()}),L.addEventListener("click",function(){b()}),M.addEventListener("click",function(){I.classList.toggle("fullscreen")});function b(){let t=document.querySelector(".container");o.querySelector(".mask").classList.toggle("show");let n=o.getElementById("chat-popup");n.classList.toggle("hidden"),n.classList.contains("fullscreen")&&n.classList.toggle("fullscreen"),n.classList.contains("hidden")?(t.style.pointerEvents="auto",document.documentElement.style.overflowY="auto"):(o.getElementById("chat-input").focus(),t.style.pointerEvents="none",document.documentElement.style.overflowY="hidden"),_(C)}async function y(t){let s=document.createElement("div");s.className="flex justify-end mb-3",s.innerHTML=`
        <div class="bg-gray-800 text-white rounded-lg py-2 px-4 max-w-[70%] text-xl">
          ${t}
        </div>
      `,l.appendChild(s),l.scrollTop=l.scrollHeight,p.value="",r.disabled=!0,r.classList.add("opacity-50","cursor-not-allowed");let n="";for await(let e of S(t)){let a=e.id,d=o.getElementById(a);e.type=="answer"?(n+=e.content,d?B(d,n):H(n,a)):e.type=="follow_up"&&T(e.content,a)}r.disabled=!1,r.classList.remove("opacity-50","cursor-not-allowed")}function H(t,s){let n=marked.parse(t),e=o.getElementById("chat-messages"),a=document.createElement("div");a.id=s,a.className="flex mb-3 text-lg",a.innerHTML=`
        <div class="bg-gray-200 text-black rounded-lg py-2 px-4 max-w-[70%]">
          <div class="markdown-content text-xl">${n}</div>
        </div>
      `,e.appendChild(a),e.scrollTop=e.scrollHeight}function B(t,s){let n=marked.parse(s),e=o.getElementById("chat-messages");t.innerHTML=`
        <div class="bg-gray-200 text-black rounded-lg py-2 px-4 max-w-[70%]">
            <div class="markdown-content text-xl">${n}</div>
        </div>
      ...
      `,s.appendChild(e),s.scrollTop=s.scrollHeight}async function j(){let t=localStorage.getItem("IPInfo");if(t)return JSON.parse(t);let n=await(await fetch("https://ipinfo.io/json")).json();return localStorage.setItem("IPInfo",JSON.stringify(n)),n}async function*S(t){let s=await j(),n="pat_AFCJg5aVP7N68oTDGStQXbzz1OYyuX3fuSXAAnyx2PxF9R06OuWoLSRkxEUjjumC",e=window.location.pathname,a=e.startsWith("/posts")?`https://blog.yuanpei.me${e}`:"",d=[{role:"user",content:t,content_type:"text"}];a!=""&&d.unshift({role:"user",content:`post_url: ${a}`,content_type:"text"});let h=await fetch("https://api.coze.cn/v3/chat",{method:"POST",headers:{Accept:"text/event-stream","Content-Type":"application/json",Authorization:`Bearer ${n}`},body:JSON.stringify({bot_id:"7437722123982389257",user_id:s.ip,stream:!0,auto_save_history:!0,additional_messages:d,custom_variables:{post_url:a},extra_params:{...s}})});if(!h.ok)throw console.error(h),new Error(`HTTP error! status: ${h.status}`);let P=h.body.getReader(),z=new TextDecoder,c="";for(;;){let{done:N,value:O}=await P.read();if(N)break;for(c+=z.decode(O);c.indexOf(`
`)!=-1;){let k=c.indexOf(`
